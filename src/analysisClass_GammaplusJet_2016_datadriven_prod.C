...
void analysisClass::Loop()
{
  ...
  //  cout<<"good vtx "<<goodPVtx<<endl;
    if(!goodPVtx){
     ...
     bool one_presel_photon = false ;
     //to change after purity studies
     if( PhotonsmearPt->size() > 1 && PhotonLoosePt->size() == 1 ){
      ...
      if(!morethanonetight) one_presel_photon = true ;
     
     }else if(PhotonLoosePt->size() == 1 ){
     	
     	one_presel_photon = true;
     
     }

          if( one_presel_photon /*nPhotonTight == 1*/ /*&& PhotonsmearPt->size() == 1*/){
       ...
       //   cout<<"idx photon "<<indexgoodpho<<endl;
               ...
               if(!HaspixelSeed->at(0/*indexgoodpho*/)){
                     if(PhotonLoosePt->at(0/*indexgoodpho*/)>= 40.){
                        bool keepmuon = true ;
                        if(nMuonsLoose != 0){ 
     ...
     gammaloose.SetPtEtaPhiE(PhotonLoosePt->at(0),PhotonLooseEta->at(0),PhotonLoosePhi->at(0),PhotonLooseEnergy->at(0));//PhotonLoosePt->at(indexgoodpho),PhotonLooseEta->at(indexgoodpho),PhotonLoosePhi->at(indexgoodpho),PhotonLooseEnergy->at(indexgoodpho));
     gamma1.SetPtEtaPhiE(PhotonLoosePt->at(0),PhotonLooseEta->at(0),PhotonLoosePhi->at(0),PhotonLooseEnergy->at(0));//PhotonLoosePt->at(indexgoodpho),PhotonLooseEta->at(indexgoodpho),PhotonLoosePhi->at(indexgoodpho),PhotonLooseEnergy->at(indexgoodpho));
    ...
    if(no_jets_ak4-nfakejet>=1  ) 
     ...
     for(int i = 0 ; i < no_jets_ak4-nfakejet; i++){
        ...
        if(!isData && jetPtGenAK4->at(sortedJetIdx[0]) > 0.){ hasgen = 1;}
        if((CorrFactors[sortedJetIdx[i]]/jetJecAK4->at(sortedJetIdx[i]))*jetPtAK4->at(sortedJetIdx[i]) >=  2. &&  isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[i]), jetNhfAK4->at(sortedJetIdx[i]), neMultAK4->at(sortedJetIdx[i]), chMultAK4->at(sortedJetIdx[i]), jetNemfAK4->at(sortedJetIdx[i]), jetMufAK4->at(sortedJetIdx[i]), jetChfAK4->at(sortedJetIdx[i]), jetCemfAK4->at(sortedJetIdx[i]),idLAK4->at(sortedJetIdx[i]), isData, hasgen,true))
	   {
...
//------------------------END met calculation------------------
 ...
 for(int i = 0 ; i < 5; i++)
     ...
     if(ak4j1.Pt()>0 && isNewonOldValidJetTight(jetEtaAK4->at(sortedJetIdx[0]), jetNhfAK4->at(sortedJetIdx[0]), neMultAK4->at(sortedJetIdx[0]), chMultAK4->at(sortedJetIdx[0]), jetNemfAK4->at(sortedJetIdx[0]), jetMufAK4->at(sortedJetIdx[0]), jetChfAK4->at(sortedJetIdx[0]), jetCemfAK4->at(sortedJetIdx[0]),idLAK4->at(sortedJetIdx[0]), isData, hasgen,false)) 
    {
     ...
     fillVariableWithValue("sigmaietaieta_photon", Photonfull5x5SigmaIEtaIEtaMapToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("CHiso_photon", PhotonphoChargedIsolationToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("NHiso_photon", PhotonphoNeutralHadronIsolationToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("Photoniso_photon", PhotonphoPhotonIsolationToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("Pt_photonSC",PhotonSCPt->at(0/*indexgoodpho*/));
     fillVariableWithValue("Eta_photonSC",PhotonSCEta->at(0/*indexgoodpho*/));
     fillVariableWithValue("Phi_photonSC",PhotonSCPhi->at(0/*indexgoodpho*/));
     fillVariableWithValue("Energy_photonSC",PhotonSCEnergy->at(0/*indexgoodpho*/));
     fillVariableWithValue("hadTowOverEm",hadTowOverEm->at(0/*indexgoodpho*/));
     fillVariableWithValue("sigmaietaiphi_photon", Photonfull5x5SigmaIEtaIPhiMapToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("E5_5_photon", Photonfull5x5E5x5MapToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("E2_2_photon", Photonfull5x5E2x2MapToken->at(0/*indexgoodpho*/));
     fillVariableWithValue("ES_Eff_photon", PhotonESEffSigmaRRMapToken->at(0/*indexgoodpho*/));
     ...
     if(isData)
       ...
       if(!isData)
       {
         fillVariableWithValue("Pt_photonGEN",photonPtGen->at(0/*indexgoodpho*/));
	 fillVariableWithValue("Eta_photonGEN",photonEtaGen->at(0/*indexgoodpho*/));
	 fillVariableWithValue("Phi_photonGEN",photonPhiGen->at(0/*indexgoodpho*/));
	 fillVariableWithValue("Energy_photonGEN",photonEnergyGen->at(0/*indexgoodpho*/));
	 fillVariableWithValue("PassGenmatching",isGenMatch->at(0/*indexgoodpho*/));
	 
	 fillVariableWithValue( "Isfakephoton", isfakephoton->at(0/*indexgoodpho*/)); 
...
//cout<<"sizes : "<< isMatch30->size()<<" "<<isMatch50->size()<<" "<<isMatch75->size()<<" "<<isMatch90->size()<<" "<<isMatch120->size()<<" "<<isMatch165->size()<<endl;
     if( isMatch30->size() > 0 && 0 < isMatch30->size() )//&& isData)
     {
      // cout<<"test core 1"<<endl;
       fillVariableWithValue("phomatchHLT_Photon30",isMatch30->at(0));// 
    ...
    if( isMatch50->size() > 0 && 0 < isMatch50->size() )//&& isData)
    {
      // cout<<"test core 2"<<endl;
       fillVariableWithValue("phomatchHLT_Photon50",isMatch50->at(0));//
     ...
     //  cout<<"test core 3"<<endl;
       fillVariableWithValue("phomatchHLT_Photon75",isMatch75->at(0));// 
     ...
     if( isMatch90->size() > 0 && 0 < (isMatch90->size() )  )// && isData)
     {
      // cout<<"test core 4 index pho "<< indexgoodpho<< " size "<<(isMatch90->size() ) <<endl;
       fillVariableWithValue("phomatchHLT_Photon90",isMatch90->at(0));//
     ...
     //  cout<<"test core 5"<<endl;
       fillVariableWithValue("phomatchHLT_Photon120",isMatch120->at(0));//
     ...
     if( isMatch165->size() > 0 && 0 < isMatch165->size() )// && isData)
     {
      // cout<<"test core 6"<<endl;
       fillVariableWithValue("phomatchHLT_Photon165",isMatch165->at(0));//
